...
def predict_speed_down(
    ...
    if bottom < roi_position + roi_position/5:
        scale_constant = 1  # scale_constant is used for manual scaling because we did not performed camera calibration
    elif bottom > roi_position + roi_position/5 and bottom < roi_position + roi_position/2:
        scale_constant = 2  # scale_constant is used for manual scaling because we did not performed camera calibration
    else:
        isInROI = False
    
    diff = abs(bottom - top)
    #print("crop 5: " + str(crop_img.shape[0]/5))
    #print("crop 3: " + str(crop_img.shape[0]/3))

    if len(bottom_position_of_detected_vehicle) != 0 and bottom \
        - bottom_position_of_detected_vehicle[0] > 0 \
        and bottom_position_of_detected_vehicle[0]  > roi_position + crop_img.shape[0]/5  \
        and bottom_position_of_detected_vehicle[0] < roi_position +crop_img.shape[0]/3  \
    ...
    # for debugging
    #print("bottom_position_of_detected_vehicle[0] with diff: " + str(bottom_position_of_detected_vehicle[0]-diff))
    #print("bottom: " + str(bottom))
    #print("roi position plus diff: " + str(roi_position +diff))
    #print("diff top bottom : " + str(bottom - top))
